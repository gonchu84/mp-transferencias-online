    ...
    string password)
{
    ...
    if (!await rd.ReadAsync())
        return (false, null, "No existe el usuario (username) en app_users.");

    var role = rd.IsDBNull(0) ? null : rd.GetString(0);
    var hash = rd.IsDBNull(1) ? "" : rd.GetString(1);

    if (string.IsNullOrWhiteSpace(hash))
        return (false, role, "El usuario existe pero password_hash está vacío.");

    var ok = BCrypt.Net.BCrypt.Verify(password, hash);
    return ok ? (true, role, null) : (false, role, "Clave incorrecta (BCrypt.Verify=false).");
}
...
app.Use(async (ctx, next) =>
{
    ...
    try
    {
        ...
        if (!ok)
        {
            await Json401(ctx, $"Credenciales inválidas. {reason}");
            return;
        }
    ...
    catch (Exception ex)
    {
        ...
        await Json401(ctx, "Error validando contra DB.");
    }
});
...
async function doLogin(){
  ...
  try{
    ...
    // api() maneja 401 y muestra login
...
async function tick(){
  ...
  // 1) me/account (si da 404, es que Render no tiene tu controller actualizado)
  let accountNote = '';
